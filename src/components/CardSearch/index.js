        ...
        <Link className={styles.link} to={`asset/${item.tokenAddr}/${item.tokenId}`}>
          <div className={styles.body}>
            <div className={styles.line}>
              ...
              {!!item.price && <div className={styles.price}>{Web3.utils.fromWei(item.price, 'ether')} BNB</div>}
            </div>
            <div className={styles.line}>
          ...
          <div className={styles.foot}>
            {!!item.highestBid && (
              <div className={styles.status}>
                ...
                Highest bid <span>{Web3.utils.fromWei(item.highestBid[1])} {item.highestBid[2] ? 'BNB' : 'SEA'}</span>
              </div>
            )}
            <div
