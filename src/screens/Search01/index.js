...
const Search = () => {
  ...
  useEffect(() => {
    if (filtersNftslikes.length > 0) {
      const leastLiked = []
        .concat(filtersNftslikes)
        .sort((a, b) => (a.likes > b.likes ? 1 : -1));

      console.log("leastLiked", leastLiked);
    }
  }, [filtersNftslikes, random]);
  const filterPrice = () => {
    var userLikesArr = [];
    if (nftsList.length > 0) {
      nftsList.map(async (value, index) => {
      ...
      console.log("userLikesArr.length", userLikesArr.length);
      console.log("userLikesArr", userLikesArr);
      setfiltersNftslikes(userLikesArr);
      setRandom(Math.random() * 10);
      console.log("filtersNftslikes.length", filtersNftslikes);
      console.log("filtersNftslikes.length", filtersNftslikes.length);
    }
  };
  ...
  // console.log("mydatasorted", oldest_to_new);
  useEffect(() => {
