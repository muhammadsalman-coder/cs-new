  ...
  const handleLikes = () => {
    ...
    if (tInfo?.token_address && account) {
      fetchViewAndLike({
        ...
        console.log(data);
        if (
          !data.likedAccounts?.find((v) => v === account?.toLowerCase()) ||
          tInfo.owner_of?.toLowerCase() === account.toLowerCase()
        ) {
        ...
        <div className={cn(styles.container)}>
          ...
          <div className={styles.details}>
            ...
            <div className={styles.nav}>
              ...
              <div className={cn(styles.stats)} onClick={handleLikes}>
                <AiFillHeart size={24} style={{ marginRight: "6px" }} />
                <div className={cn(styles.statsLink)} href="/">
                  {viewAndLikeInfo?.likes || 0}{" "}
                  {/* {viewAndLikeInfo?.likedAccounts.includes(
                    account.toLowerCase()
                  )
                    ? "UnLike"
                    : "Like"} */}
