...
const UpdateBid = ({ className, tokenInfo, bid, onConfirm, onClose }) => {
  const [inputValue, setInputValue] = useState(
    Web3.utils.fromWei(bid.price, "ether")
  );
  ...
  const handleBid = async () => {
    if (account && contracts.nftController) {
      ...
      try {
        ...
        if (
          bid.withEther &&
          parseFloat(inputValue) > Web3.utils.fromWei(bid.price)
        ) {
    ...
    if (bid.withEther) {
      if (parseFloat(inputValue) > Web3.utils.fromWei(bid.price)) {
        return (
          parseFloat(inputValue) -
          Web3.utils.fromWei(bid.price, "ether") +
          ((parseFloat(inputValue) - Web3.utils.fromWei(bid.price, "ether")) *
            feePercent) /
            100
        );
      }
  ...
  return (
    <div className={cn(className, styles.connect)}>
      ...
      <div className="table-font">
        You are about to purchase ChainCollection Art
      </div>
      <div>
        <h3>Your bid</h3>
      </div>
      ...
      <div className={cn(styles.line, styles.info)}>
        ...
        <span className="table-right">
          {convertNumber(Web3.utils.fromWei(bid.price))}{" "}
          {bid.withEther ? "BNB" : "SEA"} (
          {convertNumberToCurrencyFormat(priceInUSD)})
        </span>
      </div>
      <div className={cn(styles.line, styles.info)}>
        <span className="gray">You will pay in total</span>
        <span className="table-right">
          {convertNumber(total)} {bid.withEther ? "BNB" : "SEA"} (
