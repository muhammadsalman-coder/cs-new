  ...
  const onListHandler = async () => {
    if (!!account && contracts.nftController) {
      ...
      try {
        ...
        let res = await web3.eth.getTransactionReceipt(transactionHash);
        // console.log("Listing Tx: ", res);?
        ...
        if (metadata?.data?.imageUrl) {
          metadata.data.imageUrl =
            ...
            metadata.data.imageUrl.slice(28);
        }
        await axios.post(`${BACKEND_URL}nft-collector`, {
