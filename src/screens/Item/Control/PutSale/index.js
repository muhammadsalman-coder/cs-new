  ...
  const onListHandler = async () => {
    if (!!account && contracts.nftController) {
      ...
      try {
        ...
        let resTransConfirm = await web3.eth.getTransactionReceipt(
          transactionHash
        );
        // console.log(" PutSale Listing getTransactionReceipt: ", res);
        // console.log(" PutSale Listing transactionHash: ", transactionHash);

        let uri = await contracts.closedSeaNft.methods.tokenURI(tokenId).call();
        uri = uri.replace(PINATA_BASE_URL_FREE, PINATA_BASE_URL_GATEWAY);

        let metadata = await axios.get(uri);

        if (metadata?.data?.imageUrl) {
    ...
    <div className={cn(className, styles.sale)}>
      ...
      <div className={styles.table}>
        ...
        <div className={styles.row}>
          <div className={styles.col}>Service Fee</div>
          <div className={styles.col}>
            {selectedCurrency === CURRENCY_OPTIONS[0] ? feePercent : 0}%
          </div>
        </div>
        <div className={styles.row}>
          <div className={styles.col}>Price in USD</div>
