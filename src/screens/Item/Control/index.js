...
import stylesConfirm from "./Confirm/Confirm.module.sass";
...
const Confirm = ({
  ...
  return (
    <div className={cn(className, stylesConfirm.checkout)}>
      <div className={cn("h4", stylesConfirm.title)}>Purchase</div>
      <div className={stylesConfirm.line}>
        {!listedInfo.withEther && !approved ? (
          <button
            onClick={onApproveHandler}
            className={cn("button btn-square", stylesConfirm.button, {
              [stylesConfirm.disabled]: loading,
            })}
          >
            {loading ? "Approving..." : "Approve"}
          </button>
        ) : (
          <div className={stylesConfirm.purchase}>
            {
              // checkingBalance ? (
              // !hasSuffientBalance && (
              // <span>Loading.</span>
              // ) :
              // (console.log("hasSuffientBalance", !hasSuffientBalance),
