...
const Checkout = ({
  ...
  // console.log("shanuserinCheckiut", user);
  const total = useMemo(() => {
  ...
  console.log("tokenInfomunnaa", JSON.parse(tokenInfo.metadata));
  return (
    <div className={cn(className, styles.checkout)}>
      <div className={cn("h4", styles.title)}>Checkout</div>
      <div className={styles.title}>
        You are about to purchase an NFT from
        {/* <strong> {tokenInfo.name}</strong> */}
        <strong> {tokenInfo.metadata.split(",")[1].split(":")[1]}</strong>
      </div>
      ...
      {user?.isVerified ? (
        ...
        <div className={styles.attention}>
          ...
          <div className={styles.details}>
            <div className={styles.subtitle}>Creator not verified</div>
            {/* <div className={styles.text}>Purchase this item at your own risk</div> */}
          </div>
        </div>
      )}
