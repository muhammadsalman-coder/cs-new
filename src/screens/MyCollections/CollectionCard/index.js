...
import { deleteCollection } from "redux/action/apis/collections/deleteCollectionAction";
...
const CollectionCard = ({ collection, deletedCollection }) => {
  console.log('collectionCard', collection.nftAddress)
  const dispatch = useDispatch();
  const { deleteCollections, loading } = useSelector(
    (state) => state.deleteCollection
  );
  // console.log("delete deleteCollections", deleteCollections);

  const history = useHistory();
  //const { tokens } = useFetchCollectionTokens({
    //name: collection.name,
    //nftAddress: collection.nftAddress,
  //});
  console.log('tokens for collection', collection.tokens)
  ...
  const onDeleteHandler = () => {
    dispatch(deleteCollection({ _id: collection._id }));
    ...
    <div className={styles.root} onClick={onDetailHandler}>
        ...
        >
          ...
          {moreOpen && (
            <div className={styles.dropdown}>
              <Link
                to={`/edit-collection/${collection.name}`}
                className={styles.menuItem}
              >
                Edit collection
              </Link>
              <div className={styles.menuItem} onClick={onDeleteHandler}>
                Delete collection
              </div>
            </div>
          )}
      ...
      <div className={styles.preview}>
        {collection.avatar ? (
          ...
          <AiOutlineFileImage style={{ width: "64px", height: "64px" }} />
        )}
      </div>
      <div className={styles.footer}>
        ...
        <span className={cn("h6", styles.marginX, styles.subtitle)}>
          Explore {collection.name}
        </span>
        <span className={cn(styles.marginX, styles.subtitle)}>
