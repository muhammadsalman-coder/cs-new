...
// import useFetchCollectionNames from "utils/hooks/use-fetch-collection-names"; // comment by Muhammad Salman convert to redux
import {
...
import { fetchCollectionNames } from "redux/action/apis/collections/fetchCollectionNamesAction";
...
// } from "redux/action/apis/fetchCollectionNamesAction";
...
const Upload = () => {
  ...
  const collectionNames = useSelector(
    (state) => state.collectionNames.collectionNames
  );

  // console.log("collectionNames muhammmad slman console --", collectionNames);
  // console.log("myCollections  slman console --", myCollections);
  ...
  // const { collectionNames } = useFetchCollectionNames(); // comment by Muhammad Salman convert to redux
  ...
  useEffect(() => {
    if (account) {
      let tempCollections = [];
      const isDefaultCollection = !!myCollections.find(
        (c) =>
          c.nftAddress?.toLowerCase() ===
      ...
      if (!isDefaultCollection) {
        tempCollections = [
          `Untitled-Collection-${getCollectionHash(
            account,
            artifacts.closedSeaNft[process.env.REACT_APP_DEFAULT_CHAINID]
          )}`,
        ];
      }
  ...
  }, [account, location, myCollections, collectionNames]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const metadata = {
        imageUrl: `${PINATA_BASE_URL_GATEWAY}ipfs/${fileHash}`,
        name: values.name,
      ...
      console.log("uploadJson json", json);
      const jsonHash = await uploadJson(json);
      console.log("json uploadJson jsonHash", jsonHash);
      if (!jsonHash) {
        setLoading(false);
  ...
  const mintNFT = async (jsonHash, json) => {
    if (account && contracts.closedSeaNft) {
      ...
      console.log("transactionHx transactionHash1321", seaAmountForExemptFee);
      ...
      try {
        ...
        console.log("transactionHx transactionHash", transactionHash);

        const tx = await web3.eth.getTransactionReceipt(transactionHash);
        console.log("transactionHx ", tx);
        if (tx) {
          const mintedCount = await contracts.closedSeaNft.methods
            .tokenCountMinted()
            .call();
          toast(
          ...
          const isExisting = !!collectionNames.find(
          ...
          if (isExisting) {
            ...
            const resData = await axios.post(`${BACKEND_URL}nft-collector`, {
              selectedCat: selectedCollection,
              tokenId: parseInt(mintedCount) - 1,
              metadata: json.pinataContent,
              isOnSell: false,
              status: "pending",
            ...
            console.log("resData if", resData);
            ...
            console.log("resData else", resData);
            await postCollectionData(collectionData);
          }
          history.push("/");
          setLoading(false);
        } else {
          console.log("transactionHx else", tx);
        }
      } catch (err) {
        toast("Transaction Failed", { type: "error" });
        setLoading(false);
        console.log("transactionHx err", err);
      }
    }
  };
